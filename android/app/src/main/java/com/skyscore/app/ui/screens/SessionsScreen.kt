...
import com.skyscore.app.export.PdfExporter
...
fun SessionsScreen(viewModel: AppViewModel, state: AppUiState) {
    ...
    LazyColumn(
        ...
        // --- Feuille de route de la soiree ---
        item {
            SectionCard(
                title = "Feuille de route",
                subtitle = "Les cibles du dernier calcul Dashboard, exportables en PDF",
            ) {
                if (state.topTargets.isEmpty()) {
                    Text(
                        "Lancez d'abord une session depuis le Dashboard : la feuille de " +
                            "route reprend les cibles calculees.",
                        style = MaterialTheme.typography.bodySmall,
                        color = MaterialTheme.colorScheme.secondary,
                    )
                } else {
                    Text(
                        "%d cible(s) retenue(s) — %s, Bortle %d.".format(
                            state.topTargets.size,
                            state.params.site.name,
                            state.params.site.bortle,
                        ),
                    ...
                    onClick = {
                        scope.launch {
                            ...
                            context.startActivity(
                                Intent.createChooser(share, "Exporter la feuille de route")
                            )
                        }
                    },
                    enabled = state.topTargets.isNotEmpty(),
                ) { Text("Export PDF") }
            }
        }
