package com.skyscore.app.data.db
...
abstract class SkyScoreDatabase : RoomDatabase() {
    ...
    companion object {
        @Volatile
        private var instance: SkyScoreDatabase? = null

        fun get(context: Context): SkyScoreDatabase =
            instance ?: synchronized(this) {
                instance ?: build(context).also { instance = it }
            }

        private fun build(context: Context): SkyScoreDatabase {
            Log.i("Database", "Initialisation de la base skyscore.db")
            return Room.databaseBuilder(
                context.applicationContext,
                SkyScoreDatabase::class.java,
                "skyscore.db",
            ).build()
        }
    }
